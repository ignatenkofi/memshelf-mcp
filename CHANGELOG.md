...
- **`adapters/claude-code/check-shelve-copies.sh` — сверка копий скилла `shelve`
  командой (claude-bus#21).** Четыре копии расходились шестнадцать дней на одном
  дефекте — `git add -A` в шаге 7 — потому что их ничто не сверяло; а приёмка
  «grep не находит `git add -A`» красила бы ровно починенные копии: они называют
  команду, чтобы запретить её. Скрипт принимает любые пути и роняет копию, где
  blanket-стейджинг (`git add -A|--all|-u|--update|.`, `commit -a`) предписан —
  в code-fence или во фразе без отрицания, — либо нет ни одной строки, стейджащей
  эпизод по пути. `tests/test_shelve_skill_step7.py` пинует упакованную копию и
  красную фикстуру со старым шагом 7 — гард умеет провалиться.
...
- **Происхождение `approx_tokens` стало данными: `approx_tokens_source`
  (#79, #113; решение владельца 2026-09-01).** Число — вход головных метрик
  полки, и ничто в данных не отличало измеренное от прикинутого — и от
  неизмеренного вовсе, которое приходило арифметически работающим нулём.
  Теперь: число не передано → `approx_tokens: 0` + `source: unmeasured`
  (отсутствие замера выразимо и не выглядит измеренным нулём); передано без
  оговорок → `estimate` (честный дефолт: именно это вызывающие и передают,
  методология M0); `measured` — явное утверждение вызывающего. Источник с
  числом-которого-нет — противоречие и отказ. Поле едет во frontmatter
  (откуда rebuild рендерит реестр); ledger сохраняет шесть колонок
  shelf-spec v0 § 4.4 — колонка появится со спекой, данные копятся уже
  сейчас. Легаси-эпизоды без поля — легаси, не один из трёх исходов.
  Баннер stats называет массу оценкой там, где её читают
  (`holds ~… est.`) — дешёвая половина вреда #79 закрыта немедленно.
...
- **Публичная поверхность — §0 аудита #81 закрыт.** Три ссылки, уходившие из
  `docs/` и дававшие 404 на отрендеренном сайте (`M0.md`, `demo.md` ×2), ведут
  на GitHub абсолютно; `[project.urls]` получил `Documentation`, `Changelog`,
  `Measured results` (уедут на PyPI следующим релизом); `server.json` несёт
  `websiteUrl`. Открытыми в #81 остаются yank 0.1.0 и настройки репозитория
  (§4) — сторона владельца.

### Fixed

- **Отвергнутый вызов доезжает до клиента конвертом, а не транспортной
  ошибкой (#121).** Все инструменты типизированы как `str`, и mcp 2.x
  публикует для них `outputSchema` `{"result": …}`, который клиентский SDK
  проверяет на каждом ответе — включая собранный руками конверт ошибки в
  `call_tool`. Без `structuredContent` клиент отвергал ответ до того, как
  вызывающий видел конверт («has an output schema but did not return
  structured content»), — ровно на невалидных аргументах, в том числе на
  штатном «no shelf to work on» без `shelf_path`. Теперь конверт идёт и
  текстом, и структурной копией; регресс-тест гоняет сценарий по stdio
  настоящим клиентом — in-process контрактные тесты провода не пересекали
  и оставались зелёными.
